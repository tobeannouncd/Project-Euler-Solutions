...
def xor_encrypt(pw, message):
    out = ''
    pw_vals = []
    for c in pw:
        pw_vals.append(ord(c))
    for i in range(len(message)):
        out += chr(message[i] ^ pw_vals[i % 3])
...
def main():
    ...
    with open(filename) as f:
        message = [int(c) for c in f.read().split(',')]
    exit_flag = False
    for ia in range(ord('a'), ord('z') + 1):
        if exit_flag:
            break
        for ib in range(ord('a'), ord('z') + 1):
            if exit_flag:
                break
            for ic in range(ord('a'), ord('z') + 1):
                if exit_flag:
                    break
                pw = chr(ia) + chr(ib) + chr(ic)
                dec_message = xor_encrypt(pw, message)
                if ' the ' in dec_message:
                    print(dec_message)
                    print(pw)
                    print(sum([ord(c) for c in dec_message]))
                    exit_flag = True
    pass
